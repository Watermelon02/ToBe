 ...
 */
class DaysAdapter(var days: List<Day>) : RecyclerView.Adapter<DaysAdapter.ViewHolder>() {

    class ViewHolder(val dateBinding: ItemDayBinding) : RecyclerView.ViewHolder(dateBinding.root)
        ...
        if (days[position].date != "") {
            ...
            if (days[position].type == 2) {//为节假日
                ...
                if (days[position].lunarCalendar.isNotEmpty()) {//防止本地生成数据时为空的情况
                    holder.dateBinding.activityDateItemDayFestival.text =
                        days[position].lunarCalendar.substring(
                            days[position].lunarCalendar.length - 2,
                            days[position].lunarCalendar.length
                        )
                }
            }
            if (isCurrentMonthDay) {
                holder.dateBinding.activityDateItemDayDate.alpha = 1f
                holder.dateBinding.activityDateItemDayFestival.alpha = 1f
